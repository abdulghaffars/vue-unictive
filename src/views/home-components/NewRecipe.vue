...
async function getLastRecipe() {
  const res = await fetch(
    "https://dummyjson.com/recipes?sortBy=id&order=desc&limit=1"
  );
  if (!res.ok) return;
...
<template>
  <div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 sm:gap-8 lg:gap-16 justify-center items-center">
      <div class="w-[40%]">
        <img :src="newRecipe?.image" alt="New Recipe" class="rounded-lg" />
      </div>
      <div class="w-[60%] flex flex-col gap1 sm:gap-4 justify-center items-center sm:items-start">
        <h1 class="font-bold text-[20px] sm:text-[30px] lg:text-[60px] !mb-0 sm:mb-4 ">
          {{ newRecipe?.name }}
        </h1>
        <div class="flex items-center gap-3">
          <a-rate :value="Number(newRecipe?.rating) || 0" allow-half disabled />
